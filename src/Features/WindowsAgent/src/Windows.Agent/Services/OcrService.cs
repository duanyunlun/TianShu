...
public class OcrService : IOcrService, IDisposable
{
    ...
    private static readonly SemaphoreSlim _modelInitGate = new(1, 1);
    ...
    private async Task InitializeModelAsync()
    {
        ...
        await _modelInitGate.WaitAsync().ConfigureAwait(false);
        try
        {
            if (_model is not null)
            {
            ...
            _model = await OnlineFullModels.ChineseV4.DownloadAsync().ConfigureAwait(false);
    ...
    public async Task<(string Text, int Status)> ExtractTextFromRegionAsync(int x, int y, int width, int height, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var model = _model ?? throw new InvalidOperationException("OCR model initialization failed.");
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            if (disposing)
            {
                if (_modelInitGate.Wait(TimeSpan.FromSeconds(5)))
                {
                    try
                    {
                        _model = null;
                ...
                {
                    _logger.LogWarning("Timed out waiting for OCR model initialization gate during dispose; releasing model reference without gate.");
                    _model = null;
                }
            }
            _disposed = true;
