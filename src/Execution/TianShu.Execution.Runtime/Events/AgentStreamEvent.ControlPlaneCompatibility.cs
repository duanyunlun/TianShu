...
/// <summary>
/// 将 runtime internal `AgentStreamEvent` 显式投影为 control-plane 会话流事件。
/// Explicitly projects runtime-internal `AgentStreamEvent` values into control-plane conversation stream events.
/// </summary>
internal static class ControlPlaneConversationStreamEventCompatibility
{
    ...
    public static ControlPlaneProjectionEvent ToControlPlaneProjectionEvent(ControlPlaneConversationStreamEvent streamEvent)
    {
        ArgumentNullException.ThrowIfNull(streamEvent);

        return new ControlPlaneProjectionEvent
        {
