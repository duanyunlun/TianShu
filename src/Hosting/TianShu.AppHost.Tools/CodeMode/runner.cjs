...
function codeModeWorkerMain() {
  ...
  function ensureContentItems(context) {
    if (!Array.isArray(context.__tianshuContentItems)) {
      context.__tianshuContentItems = [];
    }
    return context.__tianshuContentItems;
  }
  ...
  async function main() {
    ...
    const context = vm.createContext({
      __tianshuContentItems: contentItems,
    });
    const helpers = createCodeModeHelpers(context, state);
    Object.defineProperty(context, '__tianshuRuntime', {
