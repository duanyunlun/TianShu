...
/// </summary>
internal static class KernelConfigObjectUtilities
{
    public static KernelConfigReadLayer CreateConfigReadLayer(
        object name,
        Dictionary<string, object?> config,
        string? disabledReason = null)
    {
        return new KernelConfigReadLayer(
            name,
            ComputeConfigObjectVersion(config),
            CloneConfigDictionary(config),
            disabledReason);
    }
    ...
    public static object? ConvertTomlValue(object? value)
    {
        return value switch
        {
            ...
            TomlTableArray tableArray => tableArray.Select(ConvertTomlTableToDictionary).Cast<object?>().ToList(),
