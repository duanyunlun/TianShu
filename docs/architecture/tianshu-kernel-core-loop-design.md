...
| 目标项目 | 状态 | 归属内容 |
| --- | --- | --- |
| `src/Contracts/TianShu.Contracts.Kernel/TianShu.Contracts.Kernel.csproj` | 未来新建 | `CoreIntent`、`StageGraph`、`KernelProposal`、`KernelOperation`、`KernelProjection`、`KernelTrace`、strategy lifecycle 等 Kernel 公共契约。 |
| `src/Core/TianShu.Kernel.Abstractions/TianShu.Kernel.Abstractions.csproj` | 已新建 | `IStableKernelCore`、`IAdaptiveOrchestrator`、`IKernelValidator`、`IStageGraphInterpreter`、trace/evaluator/strategy registry 抽象。 |
| `src/Core/TianShu.Kernel/TianShu.Kernel.csproj` | 已新建 | Stable Kernel Core、StageGraph interpreter、validator 默认实现、状态机、trace emission。 |
...
- AI 驱动的 proposal 生成和修正默认进入 `TianShu.Kernel.Adaptive`。
...
| 状态 | 允许行为 |
| --- | --- |
| `draft` | 只可验证，不可执行真实副作用。 |
| `validated` | 通过结构和边界验证，可进入 dry-run 或低风险 trial。 |
...
归属项目：`src/Contracts/TianShu.Contracts.Kernel/TianShu.Contracts.Kernel.csproj`（未来新建）。依赖来源可引用 `TianShu.Contracts.Governance`、`TianShu.Contracts.Sessions`、`TianShu.Contracts.Workflows`、`TianShu.Contracts.Primitives`。
...
归属项目：接口位于 `src/Core/TianShu.Kernel.Abstractions/TianShu.Kernel.Abstractions.csproj`（未来新建）；默认实现位于 `src/Core/TianShu.Kernel/TianShu.Kernel.csproj`（未来新建）。
...
归属项目：接口位于 `src/Core/TianShu.Kernel.Abstractions/TianShu.Kernel.Abstractions.csproj`（未来新建）；AI proposal 默认实现位于 `src/Core/TianShu.Kernel.Adaptive/TianShu.Kernel.Adaptive.csproj`（未来新建）。

```csharp
public interface IAdaptiveOrchestrator
{
    ValueTask<KernelProposalSet> ProposeAsync(
        AdaptiveOrchestrationRequest request,
        CancellationToken cancellationToken);

    ValueTask<KernelProposalSet> ReviseAsync(
        AdaptiveRevisionRequest request,
        CancellationToken cancellationToken);

    ValueTask<RecoveryProposal> ProposeRecoveryAsync(
        RecoveryOrchestrationRequest request,
        CancellationToken cancellationToken);
}

public sealed record AdaptiveOrchestrationRequest(
    CoreIntent Intent,
    KernelRunState State,
    StageGraph? CurrentGraph,
    KernelTraceSummary TraceSummary,
    IReadOnlyList<ToolDescriptor> AvailableKernelTools,
    IReadOnlyList<ToolDescriptor> AvailableCapabilityTools);
```

Adaptive Orchestration Layer 只能返回 `KernelProposalSet`。它不得返回 `RuntimeStep`，不得直接访问 Module Plane。

### 15.4 StageGraph IR 接口形态

归属项目：`src/Contracts/TianShu.Contracts.Kernel/TianShu.Contracts.Kernel.csproj`（未来新建）。既有 `TianShu.Contracts.Orchestration` 只能作为迁移输入或 bridge。
...
归属项目：`src/Contracts/TianShu.Contracts.Kernel/TianShu.Contracts.Kernel.csproj`（未来新建）。
...
归属项目：`src/Contracts/TianShu.Contracts.Tools/TianShu.Contracts.Tools.csproj`（现有）承载通用 `ITianShuTool`、`ToolDescriptor`、schema、permission、side effect、audit；`src/Contracts/TianShu.Contracts.Kernel/TianShu.Contracts.Kernel.csproj`（未来新建）承载 `IKernelTool` 返回的 Kernel 语义结果类型。
...
归属项目：接口位于 `src/Core/TianShu.Kernel.Abstractions/TianShu.Kernel.Abstractions.csproj`（未来新建）；默认解释器位于 `src/Core/TianShu.Kernel/TianShu.Kernel.csproj`（未来新建）；`ExecutionPlan` 公共契约位于 `src/Contracts/TianShu.Contracts.Execution/TianShu.Contracts.Execution.csproj`（现有）。
...
归属项目：trace / evaluation 公共契约位于 `src/Contracts/TianShu.Contracts.Kernel/TianShu.Contracts.Kernel.csproj`（未来新建）；diagnostics 投影可引用 `src/Contracts/TianShu.Contracts.Diagnostics/TianShu.Contracts.Diagnostics.csproj`（现有）；strategy registry 实现位于 `src/Core/TianShu.Kernel.Strategies/TianShu.Kernel.Strategies.csproj`（未来新建）。
...
{
    ValueTask<KernelEvaluationResult> EvaluateAsync(
...
{
    ValueTask<StrategyRecord?> FindCandidateAsync(
        CoreIntent intent,
        StrategySelectionContext context,
        CancellationToken cancellationToken);

    ValueTask<StrategyRecord> SaveDraftAsync(
        StageGraph graph,
        StrategyMetadata metadata,
        CancellationToken cancellationToken);

    ValueTask<StrategyRecord> TransitionAsync(
        string strategyId,
        StrategyLifecycleTarget target,
        StrategyTransitionEvidence evidence,
        CancellationToken cancellationToken);
}
```

没有 trace 的 strategy 不得晋升。没有 evaluation evidence 的 strategy 不得进入 `promoted`。
...
| `IKernelEvaluator` | 评估 run 与 strategy。 |
| `IStrategyRegistry` | 管理 draft、validated、trial、promoted、deprecated、rolled_back。 |
